...
fs.readdirSync("models").filter((file) => file.endsWith(".js")).forEach((file) => {
...
assert.strictEqual(context.SvmModel.train(svmPoints.map((point, index) => ({ ...point, index })), [0, 0], 0, 1, 2, 0, 0).round, 1);
